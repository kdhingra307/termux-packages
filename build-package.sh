...
termux_set_multi_arch() {
	case $TERMUX_ARCH in
		"aarch64") TERMUX_ARCH="arm";;
		"x86_64") TERMUX_ARCH="i686";;
		*) termux_error_exit "It is impossible to set multi arch for ${TERMUX_ARCH} arch, because it is 32 bit. Only 64 bit arch are supported."
	esac
}

# Run functions for normal compilation and multi-compilation
termux_normal_and_multi_compilation() {
	case "${1}" in
...
for ((i=0; i<${#PACKAGE_LIST[@]}; i++)); do
	...
	(
		...
		termux_normal_and_multi_compilation termux_step_configure

		if [ "$TERMUX_CONTINUE_BUILD" == "false" ]; then
			cd "$TERMUX_PKG_BUILDDIR"
			termux_step_post_configure
		fi
		termux_normal_and_multi_compilation termux_step_make
		termux_normal_and_multi_compilation termux_step_make_install
		cd "$TERMUX_PKG_BUILDDIR"
		termux_step_post_make_install
